...
# Enhancement Added styles for dark mode toggle and improved buttonsidebar look
def apply_custom_css()
    ...
    style
        ...
        [data-testid=stSidebar] .stButtonbutton {  Specificity for sidebar buttons 
             border-color var(--secondary-color);
             color var(--secondary-color);
             background-color transparent;
        }
         [data-testid=stSidebar] .stButtonbuttonhover {
        ...
        div[data-testid=stDownloadButton]  buttonhover {
    ...
    style
        ...
        body.dark-mode .stButtonbutton {
            border-color var(--button-border);
            color var(--button-text);
            background-color var(--button-bg);
        }
        body.dark-mode .stButtonbuttonhover {
            background-color var(--button-hover-bg);
            color var(--button-hover-text);
            border-color var(--button-hover-bg);
        }
        ...
        body.dark-mode div[data-testid=stDownloadButton]  buttonhover {
            background-color #2fad4f;
            border-color #2fad4f;
        }

         body.dark-mode [data-testid=stSidebar] .stButtonbutton {  Sidebar buttons in dark mode 
             border-color var(--secondary-color);
             color var(--secondary-color);
             background-color transparent;
        }
         body.dark-mode [data-testid=stSidebar] .stButtonbuttonhover {
             background-color var(--secondary-color);
             color var(--primary-bg);
         }
    ...
    # JavaScript to addremove 'dark-mode' class from body
...
def load_data(uploaded_file)
    ...
    logging.info(fAttempting to load file {uploaded_file.name}, extension {file_extension})
    ...
    try
        if file_extension == '.csv'
            ...
            logging.info(fSaved PDF to temporary file {tmp_file_path})
            ...
            try
                ...
                logging.warning(fTabula lattice=True failed {e_lattice}. Trying stream=True.)
                try
                    ...
                    logging.error(fTabula stream=True also failed {e_stream})
                    ...
                    if java in str(e_stream).lower() or isinstance(e_stream, FileNotFoundError)
                        ...
                        st.error(fFailed to read PDF tables using Tabula {e_stream})
                    dfs_tabula = None # Ensure it's None on failure
            finally
                # FIX Ensure temporary file is always removed
                if os.path.exists(tmp_file_path)
                    os.remove(tmp_file_path)
                    logging.info(fRemoved temporary PDF file {tmp_file_path})
            ...
            if read_successful and isinstance(dfs_tabula, list)
                ...
                if not dfs_list
                    ...
                    logging.warning(fNo non-empty tables extracted from PDF {uploaded_file.name})
                    dfs_list = None # Set back to None if only empty tables found
                else
                     logging.info(fSuccessfully extracted {len(dfs_list)} table(s) from PDF.)

        else
            st.error(fUnsupported file format '{file_extension}'. Please upload CSV, Excel, or PDF.)
            logging.error(fUnsupported file type uploaded {uploaded_file.name})
    ...
    # FIX Catch specific library errors and general exceptions during file reading
         ...
         if 'tabula' in str(ie).lower()
             st.error(Processing Error `tabula-py` library not installed or Java not foundconfigured. Cannot process PDF files.)
             logging.error(ImportError related to tabula-py or Java.)
         else
            st.error(fImport Error {ie}. Please ensure required libraries (pandas, openpyxl, tabula-py) are installed.)
            logging.error(fImportError {ie})
         return None
    except Exception as e
        st.error(fAn error occurred while reading '{uploaded_file.name}' {e})
        st.warning(Please ensure the file is valid, not password-protected or corrupted, and that Java is installed (for PDF).)
        logging.error(fFailed to load file {uploaded_file.name} {e}, exc_info=True) # Log traceback
        return None
...
with st.sidebar # ENHANCEMENT Use 'with' context manager for sidebar clarity
    ...
    # Note This primarily controls the JSCSS class switching. Streamlit's own theme
        ...
        if not st.session_state.data_loaded or uploaded_file.name != st.session_state.get('uploaded_file_name')
            ...
            if st.session_state.dataframes_list # Check if load_data returned a valid list
                ...
                if len(st.session_state.dataframes_list)  1
                    st.success(fFound {len(st.session_state.dataframes_list)} tables.)
                    table_options = {i fTable {i+1} (Shape {df.shape}) for i, df in enumerate(st.session_state.dataframes_list)}
                    ...
                    st.success(fFile '{uploaded_file.name}' loaded.)
                ...
                st.session_state.original_df = None
                st.session_state.current_df = None
                # ErrorWarning message is handled inside load_data()
...
if not st.session_state.data_loaded or st.session_state.current_df is None
    ...
    if analysis_type == 'Data Preview & Info'
        ...
        with col1
            ...
            st.write(fFile Name `{st.session_state.uploaded_file_name}`)
            if st.session_state.dataframes_list and len(st.session_state.dataframes_list)  1
                st.write(fSelected Table `Table {st.session_state.selected_df_index + 1}`)
            st.write(fRows `{df.shape[0]}`)
            st.write(fColumns `{df.shape[1]}`)
        with col2
        ...
        with st.expander(Download Data)
            ...
            st.download_button(
                ...
                data=csv_data,
                file_name=fcurrent_data_{st.session_state.uploaded_file_name.split('.')[0]}{'_T'+str(st.session_state.selected_df_index+1) if st.session_state.dataframes_list and len(st.session_state.dataframes_list)1 else ''}.csv,
                mime='textcsv',
            ...
            if not missing_summary.empty
                ...
                if cleaning_strategy == 'Impute missing values (Fill)'
                    imputation_method = st.selectbox(
                        Imputation Method,
                        ('Mean (Numeric Only)', 'Median (Numeric Only)', 'Mode (Most Frequent)', 'Constant Value', 'Forward Fill (ffill)', 'Backward Fill (bfill)'), # Added ffillbfill
                    ...
                    with st.spinner(Applying cleaning rules...)
                        ...
                        try
                            if cleaning_strategy == 'Drop rows with any missing values'
                                ...
                                action_taken = fDropped {rows_before - rows_after} rows with missing values.
                                ...
                                action_taken = fDropped {cols_before - cols_after} columns with missing values.
                                ...
                                if not cols_to_impute
                                    ...
                                    action_taken = fImputed {imputed_cols_count} column(s) using {imputation_method}.
                                    if skipped_cols
                                        st.warning(fSkipped non-numeric columns for MeanMedian or columns with no mode {', '.join(skipped_cols)}, icon=⚠️)

                            # Update the main dataframe in session state
                            st.session_state.current_df = df_cleaned
                            st.success(fCleaning applied {action_taken})
                            logging.info(fData cleaning action {action_taken})
                            st.rerun() # Rerun to update display (summary, preview)

                        except Exception as clean_e
                            st.error(fError during cleaning {clean_e})
                            logging.error(fCleaning failed {clean_e}, exc_info=True)
            else
                ...
                if hist_col
                    ...
                    with st.spinner(Generating Histogram...)
                        try
                            ...
                            st.error(fCould not generate histogram {e})
                            logging.error(fHistogram error for {hist_col} {e}, exc_info=True)
                    ...
                    if scatter_x and scatter_y
                        ...
                        with st.spinner(Generating Scatter Plot...)
                            try
                                ...
                                st.error(fCould not generate scatter plot {e})
                                logging.error(fScatter plot error ({scatter_x} vs {scatter_y}) {e}, exc_info=True)
                else
                        ...
                        # ENHANCEMENT Use spinner for potentially large heatmap render
                         with st.spinner(Generating Heatmap...)
                            try
                                ...
                                st.error(fCould not generate correlation heatmap {e_heatmap})
                                logging.error(fCorrelation heatmap error {e_heatmap}, exc_info=True)

                except Exception as corr_e
                     st.error(fCould not calculate correlations. Ensure numeric columns contain valid numbers. Error {corr_e})
                     logging.error(fCorrelation calculation error {corr_e}, exc_info=True)
